  ...
  (let* ((shift-p (not (zerop (logand mod-value sdl2-ffi:+kmod-shift+))))
    ...
    (labels ((step-next ()
             ...
             (set-note (note)
               (when (on-note)
                 ;; TODO column
                 (setf (.note (aref (.columns line) 0)) note)
                 (step-next)))
             ...
             (set-velocity-x0 (velocity)
               (when (<= velocity 7)
                 ;; TODO column
                 (let* ((column (aref (.columns line) 0)))
             ...
             (set-velocity-0x (velocity)
               (setf (.velocity (aref (.lines (.pattern self)) (.cursor-y self)))
                     (+ (logand (.velocity (aref (.lines (.pattern self)) (.cursor-y self))) #xf0)
                        velocity))
               (step-next)))
      (cond ((or (sdl2:scancode= scancode :scancode-up)
            ...
            ((or (sdl2:scancode= scancode :scancode-left)
                 (sdl2:scancode= scancode :scancode-h))
             (case (.cursor-x self)
               (4 (setf (.cursor-x self) 0))
               (5 (decf (.cursor-x self)))))
            ...
            ((or (sdl2:scancode= scancode :scancode-right)
                 (sdl2:scancode= scancode :scancode-l))
             (case (.cursor-x self)
               (0 (setf (.cursor-x self) 4))
               (4 (incf (.cursor-x self)))))
...
(defmethod close ((self pattern-module) &key abort)
  ...
  (loop for track in (.tracks (.sequencer *audio*))
        do (loop for pattern-position in (.pattern-positions track)
                 if (eql self (.pattern pattern-position))
                   do (remove-pattern track pattern-position))))
