...
(defun line-and-frame ()
  (if (playing)
      (let* ((sec-per-line (sec-per-line))
             ...
             (start-line (floor (/ (.nframes *audio*) frames-per-line)))
             (start-frame (mod (.nframes *audio*) (floor frames-per-line)))
             (end-line (floor (/ (+ (.nframes *audio*) *frames-per-buffer*) frames-per-line)))
             (end-frame (mod (+ (.nframes *audio*) *frames-per-buffer*) (floor frames-per-line))))
