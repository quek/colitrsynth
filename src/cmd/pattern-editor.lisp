...
(macrolet ((m (note key)
             `(defcmd ,(intern (format nil "INSERT-NOTE-~a" note) :cmd) (self)
                  (:bind (*pattern-editor-insert-keymap* ,key))
                (set-note self ,(intern (format nil "~a0" note)))))
           (m+1 (note key)
             `(defcmd ,(intern (format nil "INSERT-NOTE-~a+1" note) :cmd) (self)
                  (:bind (*pattern-editor-insert-keymap* ,key))
                (set-note self ,(intern (format nil "~a1" note)))))
           (m+2 (note key)
             `(defcmd ,(intern (format nil "INSERT-NOTE-~a+2" note) :cmd) (self)
                  (:bind (*pattern-editor-insert-keymap* ,key))
